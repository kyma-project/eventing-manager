...
type EventingSpec struct {
	...
	// +kubebuilder:validation:XValidation:rule="(self.type == 'EventMesh') && has(self.config.eventMeshSecret) && (self.config.eventMeshSecret != '')", message="secret cannot be empty if EventMesh backend is used"
