...
type EventingSpec struct {
	...
	// +kubebuilder:validation:XValidation:rule="self.type.contains==EventMesh && !has(self.config.eventMeshSecret)", message="can not be empty if EventMesh backend is used"
