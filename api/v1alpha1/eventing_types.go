...
type EventingSpec struct {
	...
	// +kubebuilder:validation:XValidation:rule="matches(self.type, 'EventMesh') && has(self.config.eventMeshSecret)", message="secret cannot be empty if EventMesh backend is used"
